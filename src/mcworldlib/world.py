...
class FQWorldTag(t.NamedTuple):
    ...
    obj: (
        anvil.RegionFile | level.Level
    )  # Object "owning" path , i.e., the one you .save()
...
class World:
    ...
    categories: t.Tuple[str, ...] = ("region", "entities", "poi")
    ...
    def name(self) -> str:
        return str(
            getattr(self.level, "data_root", {}).get(
                "LevelName", getattr(self.path, "name", "")
            )
        )
        ...
        for dimension in dimensions:
            for category in self.categories:
                for chunk in self.get_chunks(
                    progress=progress, dimension=dimension, category=category
                ):
    ...
    def get_chunk(
        ...
        try:
            ...
            raise anvil.ChunkError(
                f"Chunk does not exist: {chunk_coords}"
    ...
    def get_player(self, name=None):
        ...
        if name is None or name == "Player":
            try:
                ...
                raise u.MCError(
                    "Player not found in world '%s': %s" % (self.name, name)
                )
        # Multiplayer
    ...
    def save(self, path: u.AnyPath = None):
        ...
        if self.level is None:
            ...
            if not oldname == self._level_file:
                log.warning(
                    "Custom Level filename %r will not be preserved,"
    ...
    def walk(self, progress=False) -> abc.Iterator[FQWorldTag]:
    ...
    @classmethod
    def _load_level_path(
        cls, path: u.AnyFile, **kwargs
    ) -> tuple[pathlib.Path, level.Level]:
        ...
        if isinstance(path, io.FileIO) and path.name:
            ...
            return (pathlib.Path(path.name).parent, level.Level.parse(path, **kwargs))
        if not isinstance(path, (str, os.PathLike)):
            raise u.InvalidPath(path)
        path = pathlib.Path(path).expanduser()
        if path.is_file():
            # Assume level.dat itself
            return (path.parent, level.Level.load(path, **kwargs))
        if path.is_dir():
            # Assume directory containing level.dat
            return (path, level.Level.load(path.joinpath(cls._level_file), **kwargs))
